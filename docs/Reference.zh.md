...
| image | 是 | 图片水印的文件路径，需 [base64url_encode](#base64url_encode) 编码 |
| text | 是 | 文字水印内容，text和image必须传递一个<br>需 [base64url_encode](#base64url_encode) 编码 | 编码前文字内容长度不超过 `64` |
| font | 否 | 字体文件路径，需 [base64url_encode](#base64url_encode) 编码 | |
...
| 参数 | 必选 | 描述 | 取值范围 |
| - |  - |  - |  - |
| image | 是 | 要参与合并的图像文件路径，需 [base64url_encode](#base64url_encode) 编码 | |
...
| 参数 | 类型 | 说明 |
| --- | --- | --- |
| `input_uri` | `str` | 输入图像路径或 URL |
| `params` | `ProcessParams / dict / str` | 图像处理参数 |
| `out_path` | `str` (可选) | 输出图像保存路径 |
| `**kwargs` | | 透传至 `Image.save` 的额外参数 |

**异常**：超过处理限制时抛出 `ProcessLimitException`

**返回值**：默认保存至文件无返回；仅当 `out_path` 为空时返回处理后图像的二进制内容。
...
直接处理 Pillow `Image` 对象。参数和返回值与 `process_image` 一致，区别在于 `ori_im` 接受已打开的 Image 对象。
...
| --- | --- | --- |
| `img_path` | `str` | 输入图像路径 |
| `delta_h` | `float` | 像素色相与平均色相差值的阈值，范围 [0, 1]；仅差值小于此值的像素参与主色计算 |

**返回值**：十六进制颜色字符串，如 `"FFFFFF"`。
...
| 异常 | 父类 | 说明 |
| --- | --- | --- |
| `ProcessException` | `Exception` | 图像处理异常基类 |
| `ProcessLimitException` | `Exception` | 图像处理限制（文件大小/像素超限） |
| `ParamParseException` | `ProcessException` | 解析参数出现错误 |
| `ParamValidateException` | `ProcessException` | 参数校验不通过 |
...
| 枚举值 | Key | 说明 |
| --- | --- | --- |
| `LFIT` | `lfit` | 等比缩放，限制为指定 w×h 矩形内的最大图像 |
| `MFIT` | `mfit` | 等比缩放，延伸出指定 w×h 矩形外的最小图像 |
| `FIT` | `fit` | 等比缩放至矩形外最小图像，然后居中裁剪超出部分 |
| `PAD` | `pad` | 缩放为矩形内最大图像，然后用指定颜色居中填充空白 |
